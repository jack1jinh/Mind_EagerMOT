MOUNT_PATH = "/data0/JIN/ms_EagerMOT"  # in case you are mounting data storage externally
SPLIT = 'training'

#KITTI_WORK_DIR = MOUNT_PATH + "/storage/slurm/kimal/eagermot_workspace/kitti"
KITTI_WORK_DIR = MOUNT_PATH + "/storage copy/workspace/kitti"
#KITTI_DATA_DIR = MOUNT_PATH + "/storage/slurm/osep/datasets/kitti"
KITTI_DATA_DIR = MOUNT_PATH + "/storage copy/datasets/kitti"

NUSCENES_WORK_DIR = MOUNT_PATH + "/storage/slurm/kimal/eagermot_workspace/nuscenes"
NUSCENES_DATA_DIR = MOUNT_PATH + "/storage/slurm/kimal/datasets_original/nuscenes"
