...
class TransformationKitti(Transformation):
    ...
    def __init__(self, prefix, sequence, calib_folder='calib'):
        ...
        with open(file) as f:
            self.P0 = numpy.fromstring(f.readline().split(":")[1], sep=" ").reshape((3, 4))
            self.P1 = numpy.fromstring(f.readline().split(":")[1], sep=" ").reshape((3, 4))

            # Projection matrix from rectified camera coord to image2/3 coord
            self.P2 = numpy.fromstring(f.readline().split(":")[1], sep=" ").reshape((3, 4))
            self.P3 = numpy.fromstring(f.readline().split(":")[1], sep=" ").reshape((3, 4))

            # Rotation from reference camera coord to rectified camera coord
            line = f.readline()
            self.R_rect = numpy.fromstring(line[line.index(" "):], sep=" ").reshape((3, 3))

            # Rigid transform from lidar coord to reference camera coord
            line = f.readline()
            self.Tr_lidar_to_cam = numpy.fromstring(line[line.index(" "):], sep=" ").reshape((3, 4))

            line = f.readline()
            self.Tr_imu_to_lidar = numpy.fromstring(line[line.index(" "):], sep=" ").reshape((3, 4))
